...
// Ambil data lama
$q_tugas = mysqli_query($koneksi, "SELECT * FROM tugas WHERE id_tugas='$id_tugas' AND id_guru='$id_guru'");
if(mysqli_num_rows($q_tugas) == 0) {
    $_SESSION['error'] = "Tugas tidak ditemukan atau Anda tidak memiliki akses.";
    header("Location: tugas.php");
    exit;
}
$data_lama = mysqli_fetch_assoc($q_tugas);
...
if (isset($_POST['update'])) {
    ...
    // QUERY UPDATE: Tambahkan update kolom deskripsi
    $query_update = "
        UPDATE tugas 
        SET judul_tugas='$judul_tugas', 
            deskripsi='$deskripsi',
            id_kelas='$id_kelas', 
    ...
    if (mysqli_query($koneksi, $query_update)) {
        $_SESSION['sukses'] = "Tugas berhasil diperbarui!";
        header("Location: tugas.php"); 
        exit;
    } else {
        $error = "Gagal memperbarui tugas: " . mysqli_error($koneksi);
    }
}
...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Tugas: <span class="text-emerald-600"><?php echo htmlspecialchars($data_lama['judul_tugas']); ?></span></h1>
    <p class="text-slate-500 text-sm mt-1">Perbarui judul, instruksi, atau rentang waktu pengumpulan tugas.</p>
</div>
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="text" name="judul_tugas" required value="<?php echo htmlspecialchars($data_lama['judul_tugas']); ?>" 
        ...
        <div>
            ...
            <textarea name="deskripsi" required rows="4" 
                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all resize-y"><?php echo htmlspecialchars($data_lama['deskripsi'] ?? ''); ?></textarea>
        </div>
        ...
        <div class="pt-8 border-t border-slate-100 flex justify-end gap-3">
            ...
            <button type="submit" name="update" class="px-6 py-3 bg-[#10B981] hover:bg-emerald-600 text-white font-bold text-sm rounded-xl transition-colors flex items-center gap-2 shadow-md">
