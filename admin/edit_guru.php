...
if (isset($_POST['update'])) {
    ...
    // Mulai merangkai Query Update
    $query_update = "UPDATE users SET nama_lengkap='$nama', nip='$nip', username='$username', tahun_masuk='$tahun_masuk', tahun_keluar='$tahun_keluar', status='$status', id_mapel=$id_mapel";

    // Jika password diisi (artinya admin ingin mengubah passwordnya)
    if (!empty($_POST['password'])) {
        $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $query_update .= ", password='$password'";
    }

    $query_update .= " WHERE id_user='$id' AND role='guru'";

    if (mysqli_query($koneksi, $query_update)) {
        $_SESSION['sukses'] = "Data Guru dan Spesialisasi berhasil diperbarui!";
        header("Location: data_guru.php");
        exit;
    } else {
...
$q_guru = mysqli_query($koneksi, "SELECT * FROM users WHERE id_user='$id' AND role='guru'");
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Edit Data Guru</h1>
        <p class="text-slate-500 text-sm mt-1">Perbarui profil, spesialisasi, dan status masa aktif mengajar.</p>
    </div>
</div>
    ...
    <form action="" method="POST">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
                    ...
                    <div class="relative">
                        <select name="id_mapel" required class="w-full pl-4 pr-10 py-2.5 bg-indigo-50/30 border border-indigo-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:bg-white appearance-none transition-all">
                            ...
                            while($mp = mysqli_fetch_assoc($q_mapel)): 
                                $selected_mapel = ($mp['id_mapel'] == $data['id_mapel']) ? 'selected' : '';
                            ?>
                                <option value="<?php echo $mp['id_mapel']; ?>" <?php echo $selected_mapel; ?>><?php echo htmlspecialchars($mp['nama_mapel']); ?></option>
                            <?php endwhile; ?>
                        </select>
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Password <span class="text-[10px] font-normal text-rose-500">(Kosongkan jika tidak ingin diubah)</span></label>
                    ...
                    <div class="relative">
                        <select name="tahun_masuk" required class="w-full pl-4 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 focus:bg-white appearance-none transition-all">
                            ...
                            while($thn = mysqli_fetch_assoc($q_tahun)): 
                                $selected = ($thn['nama_tahun'] == $data['tahun_masuk']) ? 'selected' : '';
                            ?>
                                <option value="<?php echo $thn['nama_tahun']; ?>" <?php echo $selected; ?>><?php echo $thn['nama_tahun']; ?></option>
                            <?php endwhile; ?>
                        </select>
                        <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="tahun_keluar" required class="w-full pl-4 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 focus:bg-white appearance-none transition-all">
                            <option value="Sekarang" class="font-bold text-emerald-600" <?php echo ($data['tahun_keluar'] == 'Sekarang' || empty($data['tahun_keluar'])) ? 'selected' : ''; ?>>Sekarang (Masih Aktif Mengajar)</option>
                            <?php 
                            mysqli_data_seek($q_tahun, 0);
                            while($thn = mysqli_fetch_assoc($q_tahun)): 
                                $selected = ($thn['nama_tahun'] == $data['tahun_keluar']) ? 'selected' : '';
                            ?>
                                <option value="<?php echo $thn['nama_tahun']; ?>" <?php echo $selected; ?>><?php echo $thn['nama_tahun']; ?></option>
                            <?php endwhile; ?>
                        </select>
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="status" value="aktif" <?php echo ($data['status'] == 'aktif') ? 'checked' : ''; ?> class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                            <span class="text-sm text-slate-700 font-medium">Aktif</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="status" value="nonaktif" <?php echo ($data['status'] == 'nonaktif') ? 'checked' : ''; ?> class="w-4 h-4 text-rose-600 border-gray-300 focus:ring-rose-500">
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end gap-3">
            ...
            <button type="submit" name="update" class="px-6 py-2.5 bg-[#2563EB] hover:bg-blue-700 text-white font-bold text-sm rounded-xl transition-colors flex items-center gap-2 shadow-md">
