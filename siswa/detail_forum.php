...
// 1. TANGKAP ID TOPIK & VALIDASI AKSES KELAS
...
$query_topik = mysqli_query($koneksi, "
    SELECT ft.*, u.nama_lengkap as nama_guru, k.nama_kelas, mp.nama_mapel 
    FROM forum_topik ft
    JOIN users u ON ft.id_guru = u.id_user 
    JOIN kelas k ON ft.id_kelas = k.id_kelas 
    JOIN mapel mp ON ft.id_mapel = mp.id_mapel 
    WHERE ft.id_topik = '$id_topik' AND ft.id_kelas = '$id_kelas_siswa'
");

if (mysqli_num_rows($query_topik) == 0) { 
    header("Location: forum.php"); // Tendang jika bukan topik kelasnya
    exit; 
}
$data_topik = mysqli_fetch_assoc($query_topik);

// 2. PROSES PENGIRIMAN BALASAN BARU
if (isset($_POST['kirim'])) {
    ...
    if (!empty($isi_balasan)) {
        $q_insert = "INSERT INTO forum_balasan (id_topik, parent_id, id_user, isi_balasan) VALUES ('$id_topik', $parent_id, '$id_siswa', '$isi_balasan')";
...
// ==========================================
function tampilkanBalasan($parent_id, $level, $balasan_tree, $id_pengguna_aktif) {
    if (!isset($balasan_tree[$parent_id])) return;

    foreach ($balasan_tree[$parent_id] as $chat) {
        $is_me = ($chat['id_user'] == $id_pengguna_aktif);
        $waktu_chat = date('l, d F Y \p\u\k\u\l H:i', strtotime($chat['tgl_balas'])); 
        ...
        // Badge Role
        $badge_role = $chat['role'] == 'guru' ? '<span class="px-2 py-0.5 bg-blue-500 text-white text-[10px] font-bold rounded-full">Guru</span>' : '<span class="px-2 py-0.5 bg-sky-400 text-white text-[10px] font-bold rounded-full">Siswa</span>';
        ...
        <div class='flex flex-col mb-4 w-full {$margin_left}'>
            <div class='flex gap-4 w-full border border-slate-200 bg-white p-5 rounded-xl shadow-sm'>
                <div class='w-12 h-12 rounded-full shrink-0 flex items-center justify-center font-bold text-lg " . ($chat['role'] == 'guru' ? 'bg-[#0F9D58]' : 'bg-slate-300') . " text-white shadow-inner mt-1'>
                    " . ($chat['role'] == 'guru' ? '<i class="ph ph-chalkboard-teacher"></i>' : $inisial) . "
                </div>
                ...
                <div class='flex-1'>
                    ...
                    <p class='text-[11px] text-slate-400 font-medium mb-3'>{$waktu_chat} WIB</p>
                    ...
                    <div id='reply-form-{$chat['id_balasan']}' class='hidden mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl relative'>
                        <p class='text-xs font-bold text-slate-500 mb-2'>Membalas <span class='text-emerald-600'>" . htmlspecialchars($chat['nama_lengkap']) . "</span></p>
        ...
        tampilkanBalasan($chat['id_balasan'], $level + 1, $balasan_tree, $id_pengguna_aktif);
    }
}
?>
...
<div class="bg-transparent flex flex-col mb-8">
    ...
    <div class="bg-white border border-slate-200 px-8 py-6 rounded-xl shadow-sm mb-6 flex gap-4 relative overflow-hidden">
        
        <div class="w-14 h-14 rounded-full bg-[#0F9D58] text-white flex items-center justify-center font-bold text-2xl shrink-0 shadow-sm mt-1 relative z-10">
        ...
        <div class="flex-1 w-full relative z-10">
            <div class="flex items-center gap-2 mb-0.5">
                ...
                <span class="px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold uppercase rounded">Pembuat Topik (Guru)</span>
            </div>
            <p class="text-[11px] text-slate-400 font-medium mb-4">Diterbitkan pada <?php echo date('d F Y, H:i', strtotime($data_topik['tgl_buat'])); ?> WIB</p>
            ...
            <div id="reply-form-main" class="hidden mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl relative">
                <p class="text-xs font-bold text-slate-500 mb-2">Menambahkan balasan baru ke topik ini</p>
                <form action="" method="POST" class="flex flex-col gap-2">
        ...
        <div class="absolute -right-10 -top-10 w-32 h-32 bg-emerald-50 rounded-full opacity-50 z-0 pointer-events-none"></div>
    </div>
    ...
    <div class="flex-1 rounded-xl">
        ...
        if(isset($balasan_tree[0])) {
            tampilkanBalasan(0, 0, $balasan_tree, $id_siswa);
        } else {
            echo "<div class='text-center py-10 text-slate-400 font-medium bg-white rounded-xl border border-slate-200 border-dashed'>Belum ada partisipasi dalam diskusi ini. Jadilah yang pertama memberikan tanggapan!</div>";
        }
        ?>
    </div>
</div>
